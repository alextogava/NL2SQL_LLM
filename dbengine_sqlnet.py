...
class DBEngine:
    ...
    def execute(self, table_id, select_index, aggregation_index, conditions, lower=True):
        ...
        for tup in schema_str.split(', '):
            c, t = tup.split()
            schema[c] = t
        #select = 'col{}'.format(select_index) #Alex alterado 12/08/2023
        select = '' #Alex incluido 12/08/2023 inicio
        select_index.sort()
        for i, ind in enumerate(select_index):
            select = select + 'col{}'.format(ind)
            if i + 1 < len(select_index):
                select = select + '||'
        # Alex incluido 12/08/2023 fim
        # print()
        # print(select)
            ...
            if schema['col{}'.format(col_index)] == 'real' and not isinstance(val, (int, float)):
                try:
                    ...
                    try:
                        ...
                        pass
            if schema['col'+str(col_index)] == 'text': #Alex alterado 06/09/2023
                where_clause.append('lower(col{}) {} :col{}'.format(col_index, cond_ops[op], col_index))
            else:
