...
def process(data,tokenize):
    from transformers import BertTokenizer #Alex incluido 08/09/2023
    tokenizer = BertTokenizer.from_pretrained('bert-base-uncased') #Alex incluido 08/09/2023
    ...
    for i, one_data in enumerate(data):
        ...
        for (ii, token) in enumerate(nlu_t1):
            ...
            sub_tokens = tokenizer.tokenize(token) #Alex alterado 08/09/2023
...
def infer(nlu1,
    ...
    prob_sca, prob_w, prob_wn_w, pr_sn, pr_sc, pr_sa, pr_wn, pr_sql_i = model.beam_forward(wemb_n, l_n, wemb_h, l_hpu,
                                                                                    l_hs, tb,
                                                                                    nlu_t, nlu_tt,
                                                                                    tt_to_t_idx, nlu,
                                                                                    beam_size=beam_size,
                                                                                    knowledge=knowledge,
                                                                                    knowledge_header=header_knowledge) # Alex alterado 22/08/2023 incluido pr_sn
