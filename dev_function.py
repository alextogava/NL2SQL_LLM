...
def train(seq2sql_model, roberta_model, model_optimizer, roberta_optimizer, roberta_tokenizer, roberta_config,
          path_wikisql, train_loader):
    roberta_model.train()
    seq2sql_model.train()
    ...
    count_select_number = 0  # Alex incluido 15/08/2023
    ...
    for batch_index, batch in enumerate(tqdm(train_loader)):
        ...
        select_number_ground, select_column_ground, select_agg_ground, where_number_ground, \
        ...
        try:
            #
            where_value_index_ground = corenlp_local.get_g_wvi_bert_from_g_wvi_corenlp(punkt_to_roberta_token_indices,
        ...
        # score
        select_number_score, select_column_score, select_agg_score, where_number_score, where_column_score, \
        where_operator_score, where_value_score = seq2sql_model(natural_lang_embeddings, question_token_length,
                                                                header_embeddings,
                                                                header_token_length, header_count,
                                                                g_sn=select_number_ground, g_sc=select_column_ground,
                                                                g_sa=select_agg_ground,  # Alex alterado 15/08/2023
                                                                g_wn=where_number_ground, g_wc=where_column_ground,
                                                                g_wo=where_operator_ground,
                                                                g_wvi=where_value_index_ground,
                                                                knowledge=knowledge,
                                                                knowledge_header=knowledge_header)

        # Calculate loss & step #Alex alterado 15/08/2023 incluido select_number_score e select_number_ground
        loss = seq2sql_model_training_functions.Loss_sw_se(select_number_score, select_column_score, select_agg_score,
                                                           where_number_score,
                                                           where_column_score, where_operator_score, where_value_score,
                                                           select_number_ground, select_column_ground,
                                                           select_agg_ground,
        ...
        # Prediction
        select_number_predict, select_column_predict, select_agg_predict, where_number_predict, \
        where_column_predict, where_operator_predict, where_val_index_predict = seq2sql_model_training_functions.pred_sw_se(
            select_number_score,  # Alex alterado 15/08/2023
        ...
        where_column_predict_sorted = seq2sql_model_training_functions.sort_pr_wc(where_column_predict,
                                                                                  where_column_ground)

        sql_canonical_predict = seq2sql_model_training_functions.generate_sql_i(select_number_predict,
                                                                                # Alex alterado 15/08/2023
                                                                                select_column_predict,
                                                                                select_agg_predict,
                                                                                where_number_predict,
                                                                                where_column_predict_sorted,
                                                                                where_operator_predict,
                                                                                where_value_string_predict,
                                                                                natural_lang_utterance)

        # Calculate accuracy
        select_number_batchlist, select_col_batchlist, select_agg_batchlist, where_number_batchlist, \
        where_column_batchlist, where_operator_batchlist, where_value_index_batchlist, \
        where_value_batchlist = seq2sql_model_training_functions.get_cnt_sw_list(select_number_ground,
                                                                                 # Alex alterado 16/08/2023
                                                                                 select_column_ground,
                                                                                 select_agg_ground,
                                                                                 where_number_ground,
                                                                                 where_column_ground,
                                                                                 where_operator_ground,
                                                                                 where_value_index_ground,
                                                                                 select_number_predict,
                                                                                 # Alex alterado 16/08/2023
                                                                                 select_column_predict,
                                                                                 select_agg_predict,
                                                                                 where_number_predict,
                                                                                 where_column_predict,
                                                                                 where_operator_predict,
                                                                                 where_val_index_predict,
                                                                                 sql_canonical, sql_canonical_predict,
                                                                                 mode='train')

        logical_form_acc_batchlist = seq2sql_model_training_functions.get_cnt_lx_list(select_number_batchlist,
                                                                                      # Alex alterado 16/08/2023
                                                                                      select_col_batchlist,
                                                                                      select_agg_batchlist,
                                                                                      where_number_batchlist,
                                                                                      where_column_batchlist,
                                                                                      where_operator_batchlist,
                                                                                      where_value_batchlist)

        # print(select_col_batchlist)

        # lx stands for logical form accuracy
        # Execution accuracy test.
        execution_acc_batchlist, result_g, result_p = seq2sql_model_training_functions.get_cnt_x_list(
        ...
        # print(result_g)
        # print(result_p)
        # print(execution_acc_batchlist)

        # statistics
        average_loss += loss.item()

        # count
        count_select_number += sum(select_number_batchlist)  # Alex incluido 16/08/2023
    ...
    average_loss /= count
    select_number_acc = count_select_number / count  # Alex incluido 16/08/2023
    ...
    accuracy = [average_loss, select_number_acc, select_column_acc, select_agg_acc, where_number_acc, where_column_acc,
                where_operator_acc, where_value_index_acc, where_value_acc, logical_form_acc, execution_acc]

    return accuracy


def test(seq2sql_model, roberta_model, model_optimizer, roberta_tokenizer, roberta_config, path_wikisql, test_loader,
         mode="dev"):
    roberta_model.eval()
    seq2sql_model.eval()
    ...
    count_select_number = 0  # Alex incluido 17/08/2023
    count_select_column = 0  # count the # of correct predictions of select column
    count_select_agg = 0  # of select aggregation
    count_where_number = 0  # of where number
    count_where_column = 0  # of where column
    count_where_operator = 0  # of where operator
    count_where_value = 0  # of where-value
    ...
    for batch_index, batch in enumerate(tqdm(test_loader)):
        ...
        select_number_ground, select_column_ground, select_agg_ground, where_number_ground, \
        where_column_ground, where_operator_ground, _ = roberta_training.get_ground_truth_values(sql_canonical) #Alex alterado 17/08/2023
        # get ground truth where-value index under CoreNLP tokenization scheme. It's done already on trainset.
        ...
        # score
        _, _, _, select_number_predict,  select_column_predict, select_agg_predict, where_number_predict, sql_predict = seq2sql_model.beam_forward(
        ...
        # sort and generate
        where_column_predict, where_operator_predict, _, sql_predict = infer_functions.sort_and_generate_pr_w(
            sql_predict)
        ...
        # Cacluate accuracy # Alex alterado 21/08/2023
        select_number_batchlist, select_column_batchlist, select_agg_batchlist, where_number_batchlist, \
        where_column_batchlist, where_operator_batchlist, \
        where_value_index_batchlist, where_value_batchlist = seq2sql_model_training_functions.get_cnt_sw_list(select_number_ground, # Alex alterado 21/08/2023
            select_column_ground, select_agg_ground, where_number_ground,
            where_column_ground, where_operator_ground, where_value_index_ground, select_number_predict, # Alex alterado 21/08/2023
        ...
        logical_form_acc_batchlist = seq2sql_model_training_functions.get_cnt_lx_list(select_number_batchlist, # Alex alterado 21/08/2023
                                                                                      select_column_batchlist,
                                                                                      select_agg_batchlist,
                                                                                      where_number_batchlist,
                                                                                      where_column_batchlist,
                                                                                      where_operator_batchlist,
                                                                                      where_value_batchlist)
        # print(select_column_batchlist)
        # print(select_column_ground)
        # print(select_column_predict)

        # lx stands for logical form accuracy

        # Execution accuracy test.
        execution_acc_batchlist, _, _ = seq2sql_model_training_functions.get_cnt_x_list(
            engine, table_metadata, select_column_ground, select_agg_ground, sql_canonical, select_column_predict,
            select_agg_predict, sql_predict)

        # statistics
        # ave_loss += loss.item()

        # count
        count_select_number += sum(select_number_batchlist)  # Alex incluido 22/08/2023
        ...
        count_curr_batchlist = [select_number_batchlist, select_column_batchlist, select_agg_batchlist, where_number_batchlist,
                                where_column_batchlist, where_operator_batchlist, where_value_batchlist,
                                logical_form_acc_batchlist, execution_acc_batchlist] # Alex alterado 22/08/2023
        count_batchlist.append(count_curr_batchlist)

        # print(count)

    # ave_loss /= cnt
    select_number_acc = count_select_number / count  # Alex incluido 22/08/2023
    select_column_acc = count_select_column / count
    select_agg_acc = count_select_agg / count
    where_number_acc = count_where_number / count
    where_column_acc = count_where_column / count
    ...
    accuracy = [None, select_number_acc, select_column_acc, select_agg_acc, where_number_acc,
                where_column_acc, where_operator_acc, where_value_index_acc,
                where_value_acc, logical_form_acc, execution_acc] # Alex alterado 22/08/2023
