...
def Loss_sw_se(s_sn, s_sc, s_sa, s_wn, s_wc, s_wo, s_wv, g_sn, g_sc, g_sa, g_wn, g_wc, g_wo, g_wvi): #Alex alterado 15/08/2023
    """
    ...
    loss += F.cross_entropy(s_sn, torch.tensor(g_sn).to(device)) #Alex incluido 15/08/2023
...
def pred_sw_se(s_sn, s_sc, s_sa, s_wn, s_wc, s_wo, s_wv): #Alex alterado 15/08/2023
    pr_sn = pred_sn(s_sn) #Alex incluido 15/08/2023
    #pr_sc = pred_sc(s_sc)
    pr_sc = pred_sc(pr_sn, s_sc) #Alex alterado 15/08/2023
    pr_sa = pred_sa(s_sa)
    pr_wn = pred_wn(s_wn)
    ...
    return pr_sn, pr_sc, pr_sa, pr_wn, pr_wc, pr_wo, pr_wvi #Alex alterado 15/08/2023
...
def generate_sql_i(pr_sn, pr_sc, pr_sa, pr_wn, pr_wc, pr_wo, pr_wv_str, nlu): #Alex alterado 15/08/2023
    pr_sql_i = []
    for b, nlu1 in enumerate(nlu):
        ...
        for i_sn in range(pr_sn[b]):
            cols.append(pr_sc[b][i_sn])
        # Alex incluido 15/08/2023 fim

        #pr_sql_i1 = {'agg': pr_sa[b], 'sel': pr_sc[b], 'conds': conds}
        pr_sql_i1 = {'agg': pr_sa[b], 'sel': cols, 'conds': conds} #Alex alterado 15/08/2023
...
def get_cnt_sw_list(g_sn, g_sc, g_sa, g_wn, g_wc, g_wo, g_wvi,
    ...
    """
    cnt_sn = get_cnt_sc_list(g_sn, pr_sn) #Alex incluido 16/08/2023
    #cnt_sc = get_cnt_sc_list(g_sc, pr_sc)
    cnt_sc = get_cnt_sc_list_2(g_sc, pr_sc) #Alex alterado 05/09/2023
...
#Alex incluido 05/09/2023 inicio

def get_cnt_sc_list_2(g_sc, pr_sc):
    cnt_list= []
    for b, g_sc1 in enumerate(g_sc):
...
def get_cnt_lx_list(cnt_sn1, cnt_sc1, cnt_sa1, cnt_wn1, cnt_wc1, cnt_wo1, cnt_wv1): #Alex alterado 16/08/2023
    # all cnt are list here.
    cnt_list = []
    cnt_lx = 0
    for csn, csc, csa, cwn, cwc, cwo, cwv in zip(cnt_sn1, cnt_sc1, cnt_sa1, cnt_wn1, cnt_wc1, cnt_wo1, cnt_wv1): #Alex alterado 16/08/2023
        if csn and csc and csa and cwn and cwc and cwo and cwv: #Alex alterado 16/08/2023
            cnt_list.append(1)
        else:
            cnt_list.append(0)
