...
def get_ground_truth_values(canonical_sql_queries):
    ground_select_number = [] #Alex incluido 15/08/2023
    ...
    for _, canonical_sql_query in enumerate(canonical_sql_queries):
        ...
        sel_num = canonical_sql_query['sel'] #Alex incluido 15/08/2023
        if not canonical_sql_query["agg"] < 0:
            ground_select_number.append(len(sel_num)) #Alex incluido 15/08/2023
    ...
    return ground_select_number, ground_select_column, ground_select_aggregate, ground_where_number, ground_where_column,\
           ground_where_operator, ground_where_value #Alex alterado 15/08/2023
...
def get_roberta_output(model_roberta, tokenizer, nlu_t, headers, max_seq_length):
    ...
    #check, _, all_encoder_layer = model_roberta(input_ids=all_input_ids, attention_mask=all_input_mask, output_hidden_states=
    output = model_roberta(input_ids=all_input_ids, attention_mask=all_input_mask, output_hidden_states=True)

    #all_encoder_layer = list(all_encoder_layer)
    all_encoder_layer = output.hidden_states        
