...
class Seq2SQL_v1(nn.Module):
    def __init__(self, iS, hS, lS, dr, n_cond_ops, n_agg_ops, old=False):
        ...
        self.max_wn = 4
        self.max_sn = 4 #Alex incluido 14/08/2023
        self.n_cond_ops = n_cond_ops
        self.n_agg_ops = n_agg_ops

        self.snp = SNP(iS, hS, lS, dr) #Alex incluido 14/08/2023
    ...
    def forward(self, wemb_n, l_n, wemb_h, l_hpu, l_hs,
                g_sn=None, g_sc=None, g_sa=None, g_wn=None, g_wc=None, g_wo=None, g_wvi=None, #Alex alterado 15/08/2023
                show_p_sc=False, show_p_sa=False,
                show_p_wn=False, show_p_wc=False, show_p_wo=False, show_p_wv=False,
                knowledge=None,
                knowledge_header=None,
                show_p_sn=False): #Alex alterado 14/08/2023

        # sn Alex incluido 15/08/2023 inicio
        s_sn = self.snp(wemb_n, l_n, wemb_h, l_hpu, l_hs, show_p_sn=show_p_sn,
        ...
        if g_sn:
            ...
            pr_sn = pred_sn(s_sn)
        # wn Alex incluido 15/08/2023 fim
    ...
    def beam_forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, tb,
                     ...
                     beam_size=4,
                     show_p_sn=False,  # Alex alterado 17/08/2023
                     show_p_sc=False, show_p_sa=False,
                     show_p_wn=False, show_p_wc=False, show_p_wo=False, show_p_wv=False,
                     knowledge=None,
                     knowledge_header=None):
        ...
        # Alex incluido 15/08/2023 inicio
        s_sn = self.snp(wemb_n, l_n, wemb_hpu, l_hpu, l_hs, show_p_sn=show_p_sn,
                        knowledge=knowledge, knowledge_header=knowledge_header)
        #pr_sn = F.softmax(s_sn, dim=-1).detach().to('cpu').numpy()

        # pr_sn_based_on_prob = []
        # for b, prob_sn1 in enumerate(pr_sn):
        #     pr_sn_based_on_prob.append(argmax(prob_sn1[0]))

        pr_sn = pred_sc_beam(s_sn, beam_size)
        pr_sn_based_on_prob = [sublista[0] for sublista in pr_sn]
        # Alex incluido 15/08/2023 fim
        ...
        # Alex incluido 24/08/2023 inicio
        pr_sc_best = []
        for i, count in enumerate(pr_sn_based_on_prob):
            selected_items = pr_sc_beam[i][:count]
            pr_sc_best.append(selected_items)
        # Alex incluido 24/08/2023 fim
        ...
        return prob_sca, prob_w, prob_wn_w, pr_sn_based_on_prob, pr_sc_best, pr_sa_best, pr_wn_based_on_prob, pr_sql_i # Alex alterado 24/08/2023



#Alex incluido 14/08/2023
...
#         self.softmax_dim2 = nn.Softmax(dim=2)
# 
#     def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, show_p_sc, penalty=True, predict_select_column=None,
#                 knowledge=None,
#                 knowledge_header=None):
#         # Encode
#         mL_n = max(l_n)
...
#         p_n = p_n.unsqueeze(3)  # [b, hs, n] -> [b, hs, n, 1]
#         c_n = torch.mul(wenc_n, p_n).sum(2)
# 
...
class SAP(nn.Module):
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, pr_sc, show_p_sa=False,
        ...
        if type(pr_sc[0]) == list: #Alex alterado 11/08/2023
            var_pr_sc = [pr_sc[0][0]]
        else:
            var_pr_sc = pr_sc
        wenc_hs_ob = wenc_hs[list(range(bS)), var_pr_sc] #Alex alterado 11/08/2023
