...
def generate_sql_q1(sql_i1, tb1):
    ...
    sn = len(sql_i1['sel'])
    select_header = []
    # Alex alterado 22/08/2023 inicio
    for i in range(sn):
        select_header.append(headers[sql_i1['sel'][i]])
    #select_header = headers[sql_i1['sel']]

    select_header_2 = ", ".join(select_header)

    if select_agg:
        sql_query_part1 = f'SELECT {select_agg}({select_header_2}) '
    else:
        sql_query_part1 = f'SELECT {select_agg}{select_header_2} '
    # Alex alterado 22/08/2023 fim
